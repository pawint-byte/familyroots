...
-   **Connection Request Routing**: User connection requests now carry full tree context (`targetTreeId`, `targetTreeName`) so both sender and receiver know which tree the request targets. The public profile connection dialog includes a tree selector to specify which tree to connect to. Incoming requests show "[Name] wants to connect to [Tree Name] as [Relationship]" with clickable tree links. Outgoing requests display full context with status tracking (Pending/Approved/Declined/Cancelled) and cancel capability. When approved, the system routes the connection to the specific target tree rather than defaulting to the first tree. API: `GET /api/users/:userId/trees/public` (tree list for selector), `POST /api/user-connection-requests/:id/cancel` (cancel pending outgoing). Components: `pending-connections.tsx` (incoming), `outgoing-requests.tsx` (outgoing/sent), `public-profile.tsx` (tree selector in connection dialog).
