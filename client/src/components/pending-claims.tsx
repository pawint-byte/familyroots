...
export function PendingClaimsSection() {
  ...
  const [mergeData, setMergeData] = useState<{
    ...
    existingMemberTreeName: string;
    targetMemberTreeName: string;
  } | null>(null);
    ...
    onSuccess: (data: any) => {
      if (data?.status === 'merge_available') {
        setMergeData({
          ...
          existingMemberTreeName: data.existingMemberTreeName || "Unknown Tree",
          targetMemberTreeName: data.targetMemberTreeName || "Unknown Tree",
        });
        setShowMergeDialog(true);
        toast({
          title: "Claim approved",
          description: "This person has a profile in another tree. You can merge them now.",
        });
      } else {
        toast({
          title: "Claim approved",
  ...
  if ((!pendingClaims || pendingClaims.length === 0) && !showMergeDialog) {
    return null;
  }
    ...
    <>
      ...
      {mergeData && (
        <MemberMergeDialog
          ...
          memberA={mergeData.existingMember}
          memberB={mergeData.targetMember}
          memberATreeName={mergeData.existingMemberTreeName}
          memberBTreeName={mergeData.targetMemberTreeName}
          onMergeComplete={() => {
